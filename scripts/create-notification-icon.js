...
const cx = width / 2;
const cy = height / 2;
const radius = width / 2 - 2; // slight inset so circle doesn't clip edge

// Raw image: filter byte + RGBA per pixel
...
for (let y = 0; y < height; y++) {
  ...
  for (let x = 0; x < width; x++) {
    ...
    raw[base + 1 + x * 4] = 255; // R
    raw[base + 2 + x * 4] = 255; // G
    raw[base + 3 + x * 4] = 255; // B
    raw[base + 4 + x * 4] = inCircle ? 255 : 0; // A
  }
}
...
fs.writeFileSync('assets/images/notification-icon.png', png);
console.log('Created assets/images/notification-icon.png, bytes:', png.length);
